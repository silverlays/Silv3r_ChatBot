...
class BotController():
  def __init__(self):
    shared.client_handler = ClientHandler()
  ...
  def execute_thread(self):
    ...
    shared.client_handler.login()

    for _ in range(100):
      if shared.client_handler.tmi_client.logged: break
      else: time.sleep(0.1)
    if not shared.client_handler.tmi_client.logged: raise ConnectionError("Cannot connect! Try to restart the bot.")
    ...
    while True:
      ...
      if shared.window_closed:
        for channel in shared.main_window.get_channels(): self.loop.run_until_complete(shared.client_handler.part(channel))
        shared.client_handler.logout()
        break
      time.sleep(0.1)
  ...
  def join_channel(self, channel: str, auto=False):
    ...
    self.loop.run_until_complete(shared.client_handler.send_message(channel, shared.settings['bot_settings']['welcome_message']))
    self.loop.run_until_complete(shared.client_handler.join(channel))
  ...
  def leave_channel(self, channel: str):
    ...
    self.loop.run_until_complete(shared.client_handler.send_message(channel, shared.settings['bot_settings']['part_message']))
    self.loop.run_until_complete(shared.client_handler.part(channel))
  ...
  def send_to_channel(self, channel: str, message: str):
    self.loop.run_until_complete(shared.client_handler.send_message(channel, message))
